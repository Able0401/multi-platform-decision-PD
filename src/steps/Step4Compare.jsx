...
export default function Step4Compare() {
  ...
  const remaining = Math.max(0, STICKER_LIMIT - placedCount);

  // Aggregate sticker counts across all voters: { "<participantId>:<itemId>": [voterId,...] }
  ...
  const toggleSticker = (targetParticipantId, targetItemId) => {
    ...
    if (remaining <= 0) {
      window.alert(
        "All 3 stickers used. Remove one first. (스티커 3장을 모두 사용했습니다. 하나를 빼고 시도하세요.)"
      );
      return;
    }
    dispatch({
        ...
        <div className="mb-3 flex items-center justify-between">
          <div>
            ...
            <div className="text-[11px] text-ink-500">
              비교·투표 — Click a component to place a sticker (스티커 부착)
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            />
            <StickerCounter remaining={remaining} placed={placedCount} />
...
function StickerCounter({ remaining, placed }) {
  return (
    <div className="flex items-center gap-1 rounded-md bg-ink-50 px-2.5 py-1">
      <span className="text-[11px] text-ink-500">Stickers</span>
      <div className="flex gap-0.5">
        {Array.from({ length: STICKER_LIMIT }).map((_, i) => (
          <span
            key={i}
            className={`h-3 w-3 rounded-full ${
              i < placed ? "bg-sticker" : "border border-ink-300"
            }`}
          />
        ))}
      </div>
      <span className="text-[11px] tabular-nums text-ink-700">
        {placed}/{STICKER_LIMIT}
      </span>
    </div>
  );
}
            ...
            return (
              <button
                ...
                className={`relative w-full cursor-pointer rounded-card text-left transition ${
                  youPlaced ? "ring-2 ring-sticker" : "hover:ring-1 hover:ring-accent/40"
                }`}
              >
                <ComponentCard
